...
1. `handleChatMember` detects `left|kicked → member|restricted`, filters bots/self/channels/disallowed-chats, calls `startCaptcha`; the `join` event is recorded only when `startCaptcha` wins the kickoff (Telegram delivers most joins via BOTH `chat_member` and `new_chat_members` — recording before the dedup double-counts joins and shows phantom «в процессе» in stats). `message.new_chat_members` is the fallback path (some group types) — it also carries the forum `threadID` (`IsTopicMessage`), so forum captchas land in the right topic.
...
Per-chat opt-in (`spam_check_enabled`, default off; needs `GROQ_API_KEY` and/or `GIGACHAT_AUTH_KEY`). Provider chain (`classifySpam`): Groq primary (12s sub-budget inside the 30s check ctx); on ANY Groq error — typically its per-minute rate limit — fall back to GigaChat; Groq unset → GigaChat alone; all failed → fail-open. `internal/gigachat`: OAuth token cached ~30 min (+one forced refresh on 401), НУЦ Минцифры root CA embedded via go:embed (Sber TLS endpoints fail verification without it), robust probability parser (no guaranteed JSON mode); both clients share `groq.SystemPrompt`. Live check: `GIGACHAT_AUTH_KEY=... go test -run TestLive ./internal/gigachat -v`. Hook at the tail of `handleGroupMessage`: messages from non-whitelisted users go to the LLM (text/caption + FACTS about media — the files themselves never leave; forward origin; author name, membership age, message total) → spam probability 0–100. Whitelist skips analysis: per-chat `UserMessageTotal > spam_whitelist_msgs` (default 5) first (cheap, covers regulars), then cross-chat trust via `UserMessageTotalsByChat` — наговорил на белый список в любом ОДНОМ chatAllowed-чате бота, доверяем везде (фильтр по ALLOWED_CHATS против фарма доверия в постороннем чате; в facts для LLM идёт пер-чатовый total); owners, chat admins (cached 6h in `adminCache`, invalidated on every `chat_member` event). Probability ≥ `spam_threshold` (default 90) → reply with «мне кажется, это спам» + «Да, спам»/«Нет, не спам» buttons, persisted to `spam_votes`.

Voting: one ballot per voter (re-press switches), author excluded, trust gate — ballots count only from voters with `UserMessageTotal > spam_whitelist_msgs` in THIS chat (deliberately per-chat, no cross-chat trust; fail-closed on DB error; otherwise a couple of fresh sock-puppet accounts could clear the plashka or rubber-stamp a false LLM verdict into a global ban), margin of `spam_vote_margin` (per-chat, default 3, presets 2/3/5) decides — |за−против| ≥ N; an admin/owner vote decides instantly («золотой голос», no trust gate). Verdict executor starts with `TakeSpamVote` (atomic DELETE, first caller wins — double-click/race safe). Spam → `banRevoke` (BanChatMember with `RevokeMessages:true`, wipes ALL author's messages) + the bot's own greeting for that user (looked up via the `greetings` table, populated by `maybeSendGreeting`) + `spamban` event; not-spam → delete only the bot's message. Spam verdict also feeds the GLOBAL spammer base: `AddSpamBanned` + `banEverywhere` (single-shot BanChatMember — NOT retryTG, «нет прав» перманентна и лестница ретраев растянула бы обход — in every other allowed group from the `chats` registry; runs in a goroutine AFTER the plashka is removed; `spamban` event only in the verdict chat — иначе статистика задвоится). A join by a known spammer (`IsSpamBanned` in `onUserJoined`) gets an instant `banRevoke` instead of a captcha; on ban failure falls back to the normal captcha; dedup via `BeginKickoff` held for +1 min after the ban (иначе дубль new_chat_members задвоит spamban). Manual admin unban anywhere (`kicked→*` by a foreign non-bot hand in `handleChatMember`) forgives: `DeleteSpamBanned`. Owner DM notifications (per-owner toggle `owner_settings.spam_notify`, «🔔» button in the main DM menu, `menu:spamnotify`): suspicion = forward of the flagged message + score card (`notifySpamSuspicion`), verdict = who decided + per-name «за/против» ballots (`ListBallots` read BEFORE `TakeSpamVote` — Take wipes them) + list of chats cross-banned (`notifySpamVerdict`). `spamVoteSweepLoop` (hourly + at startup) closes votes older than 24h — past 48h Telegram refuses bot deletions entirely, so never let them age. Groq errors/timeouts are fail-open (message stays, Warn logged). Logging is deliberately verbose (owners-only surface): every check logs the full `facts` string sent to Groq + prob; every button press logs `spam vote ballot` with the voter's name/@username/id and the running tally. Groq call runs in a goroutine on `runCtx`; in-flight dedup per chat:user + one pending vote per author max. Live prompt check: `GROQ_API_KEY=... go test -run TestLive ./internal/groq -v`.
