...
func (b *Bot) handleMuteCommand(ctx *th.Context, message telego.Message) error {
	...
	// Мьют команды строже текст-онли карантина и полностью его заменяет
	// (иначе на до-таймауте /unmute карантинный рестрикт «всплыл» бы обратно
	// из нетронутой строки, а цензор продолжил бы чистить админское решение).
...
const groupCommandsList = "/spam — репорт на спамера (реплаем; могут все участники с историей сообщений; репорт админа банит сразу)\n" +
	...
	"/fullunban — снять баны во всех моих чатах и вычеркнуть из базы спамеров\n" +
	"/unmute — снять мьют (список или @username)\n" +
	"/whitelist — впускать без капчи (список или @username)\n" +
	"/trust — досрочно снять карантин новичка (реплаем или @username)"
...
// воронке). Reply-wait — по той же причине: таймаут серии кикнул бы за
