...
- 💬 **Требовать ответа** (опционально, per chat, 30/60/90/120 сек) — второй шаг проверки: новичок должен что-нибудь написать в чат после приветствия, иначе кик (как с капчей: несколько провалов → бан). Написанное сразу проходит ИИ-анализ.
...
Пресеты антиспама (белый список / перевес голосов) и секунд ожидания ответа показываются только при включённой фиче.
...
```
вход юзера ──► мут СРАЗУ (restrict) ──► пауза ~3 c ──► сообщение-капча
