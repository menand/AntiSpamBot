...
- 🛡 **Команды `/kick` и `/ban`** — для админов чата и владельцев бота (остальным «не балуйся»). Работают по `@username`, по реплаю на сообщение юзера или на приветствие бота о нём. Банит/кикает и стирает все сообщения цели, а также приветствие бота и активную плашку голосования о нём.
...
**Настройки чата — только через DM-меню** «⚙️ Настройки» в разделе `/chats` → выбираешь чат. `/kick` и `/ban` — единственные команды, работающие в группе (в «/»-меню их нет, поле ввода не засоряется).
