...
Последний релиз — **v1.6.0** (2026-07-18): команды `/unban`, `/unmute` и `/whitelist` (плашка «10 последних» с кнопками или сразу по `@username`), доверенные пользователи входят без капчи, в тихом режиме капча со второй попытки шлётся публично с кнопкой «Впустить», команда `/whatsnew` и разовое ЛС-оповещение владельцам и админам чатов при выходе новой версии.
...
| `/whatsnew`, `/whatnew` | в ЛС и в группе | всем | список изменений последних версий бота; в группе — эфемерно, видно только спросившему |
...
| `/help` | в группе | всем | эфемерная справка по командам |
