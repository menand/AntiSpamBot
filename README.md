...
- 🔔 **Уведомления владельцу в ЛС** — отдельно спам-подозрения/вердикты (🔔) и кики/баны/проходы капчи (🛡): причина кика с выбранным и верным вариантом капчи, молчание, `/kick`/`/ban`, глобальная база, проход капчи с выбранным вариантом.
...
Последний релиз — **v1.6.1** (2026-07-18): оповещения о новых версиях теперь можно отключить индивидуально (кнопка «🆕» в /start-меню, по умолчанию включены), а `/help` показывает полный список всех команд — и в группе, и в ЛС. До этого в **v1.6.0**: команды `/unban`, `/unmute` и `/whitelist` (плашка «10 последних» с кнопками или сразу по `@username`), доверенные пользователи без капчи, публичная капча со второй попытки в тихом режиме, `/whatsnew` и ЛС-оповещения о новых версиях.
...
Периоды статистики в меню: **Сегодня / Вчера / Неделя / Месяц / Всегда**.
